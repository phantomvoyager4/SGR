...
export default function Home() {
  ...
  const activityBased = [GAMES.find((g) => g.id === "rdr"), GAMES.find((g) => g.id === "call-of-juarez")];
    ...
    <div className="max-w-[1400px] mx-auto fade-up">
      ...
      >
        ...
        <div className="relative p-10 max-w-[60%]">
          ...
          <h1 className="font-display font-black text-[56px] leading-[1] mb-5 tracking-tight">Cyberpunk 2077</h1>
          <p className="text-[15px] leading-relaxed mb-7 max-w-[520px]" style={{ color: "var(--text-dim)" }}>
            Biorąc pod uwagę Twoje 450 godzin spędzonych w Wiedźminie 3 i zamiłowanie do gier RPG z otwartym światem, ta przesiąknięta neonami dystopia będzie Twoją kolejną obsesją.
          </p>
      ...
      <section data-testid="recommended-section" className="mb-16">
        <div className="flex items-end justify-between mb-8 flex-wrap gap-4">
          <div>
            <h2 className="font-display font-black text-[36px] leading-none mb-2">Rekomendowane dla Ciebie</h2>
            <p className="text-[14px]" style={{ color: "var(--text-dim)" }}>Na podstawie Twojego stylu gry…</p>
          </div>
          <div className="flex items-center gap-2">
            <span className="font-mono text-[10px] uppercase tracking-wider mr-2" style={{ color: "var(--text-faint)" }}>
              Filtruj:
            </span>
            {CHIPS.map((c) => (
              <button
                ...
                style={{
                  ...
                  border: filter === c ? "1px solid var(--teal)" : "1px solid var(--border)",
                }}
              >
                {c}
              </button>
      ...
      <section data-testid="activity-section" className="mb-10">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <div className="font-display font-bold text-[20px]">Na podstawie Twojej ostatniej aktywności…</div>
            <div className="text-[13px]" style={{ color: "var(--text-dim)" }}>
              Ponieważ niedawno grałeś w <span style={{ color: "var(--teal)" }}>Red Dead Redemption 2</span>
            </div>
          </div>
        </div>
            ...
            return (
              ...
              >
                <img
                  ...
                  onError={(e) => { e.target.src = `https://placehold.co/200x280/0F1624/5EEAD4?text=${encodeURIComponent(g.title)}`; }}
                />
                <div className="flex-1 min-w-0">
                  <div className="font-display font-bold text-[17px] mb-1 truncate">{g.title}</div>
                  <div className="text-[12px] mb-3 line-clamp-2" style={{ color: "var(--text-dim)" }}>
                    Przygodowa gra akcji z 2010 roku; wyprodukowana przez Rockstar San Diego.
                  </div>
                  <div className="flex items-center gap-3">
