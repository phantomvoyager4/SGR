...
export default function Steam() {
  ...
  return (
    <div className="max-w-[1000px] mx-auto fade-up">
      <h1 className="font-display font-black text-[44px] leading-none mb-3">Integracja Steam</h1>
      <p className="text-[15px] mb-10" style={{ color: "var(--text-dim)" }}>
        Połącz swoje konto Steam, aby importować bibliotekę i otrzymywać spersonalizowane rekomendacje.
      </p>
      ...
      <div
        ...
        style={{
          background: "linear-gradient(135deg, rgba(13, 148, 136, 0.12), rgba(5, 8, 15, 0.5))",
          borderColor: "var(--teal-dim)",
        }}
      >
        <div className="flex items-start gap-5 mb-6">
          <div className="w-14 h-14 rounded-full grid place-items-center glow-teal"
               style={{ background: "rgba(94, 234, 212, 0.15)", border: "1px solid var(--teal)" }}>
            <CheckCircle2 size={24} style={{ color: "var(--teal)" }} />
          </div>
          <div className="flex-1">
            <div className="font-display font-bold text-[24px] mb-1">Konto Steam połączone</div>
            <div className="text-[13px]" style={{ color: "var(--text-dim)" }}>
              Zalogowano jako <span style={{ color: "var(--teal)" }}>{USER.username}</span> · Ostatnia synchronizacja: {lastSync}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            ...
            { label: "Łączny czas gry", value: `${USER.totalHours.toLocaleString("pl-PL")}h` },
            { label: "Osiągnięcia", value: 782 },
          ].map((s, i) => (
            <div
              key={i}
              className="p-4 rounded-xl border"
              style={{ background: "rgba(5, 8, 15, 0.4)", borderColor: "var(--border)" }}
            >
              <div className="font-mono text-[10px] uppercase tracking-wider mb-1" style={{ color: "var(--text-faint)" }}>
                {s.label}
              </div>
              <div className="font-display font-black text-[22px]">{s.value}</div>
            </div>
          ))}
        </div>
      ...
      >
        <h3 className="font-display font-bold text-[18px] mb-4">Ustawienia synchronizacji</h3>
        <div className="space-y-4">
          {[
            { label: "Automatyczna synchronizacja biblioteki", desc: "Co 24 godziny", on: true },
            { label: "Importuj czas gry", desc: "Aktualizowany w czasie rzeczywistym", on: true },
            { label: "Udostępniaj osiągnięcia", desc: "Publiczny profil dla rekomendacji", on: false },
          ].map((opt, i) => {
            const [on, setOn] = [opt.on, () => {}];
            return (
              <ToggleRow key={i} {...opt} testid={`toggle-${i}`} />
            );
          })}
        </div>
      </div>
  ...
  return (
    <div className="flex items-center justify-between py-3 border-b last:border-0" style={{ borderColor: "var(--border)" }}>
      <div>
        ...
        <div className="text-[12px]" style={{ color: "var(--text-faint)" }}>{desc}</div>
      </div>
      <button
        data-testid={testid}
        onClick={() => setOn(!on)}
